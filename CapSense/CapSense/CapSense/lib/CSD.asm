...
.InitCycle:
   mov   [X + CSD_baSnsDebounce], CSD_Debounce
   mov   [X + <CSD_baLowBaselineReset], CSD_LowBaselineReset
   dec   X
   jnc   .InitCycle
...
.Continue:
   ...
   mov   A, [CSD_wADC_Result+1] ; A = (bResolution-9)
   index CSD_Max_ADC_Value_MSB_Table
   mov   [CSD_MaxADCValueMSB], A ; Store MSB of max Raw data value   
...
;---------------------------------
; Table for MSB of Max Raw data Values
;---------------------------------
.LITERAL
CSD_Max_ADC_Value_MSB_Table:
...
ENDIF
   ...
   mov   reg[CSD_AMUX_CFG_REG], 0                                            ; Disable external cap

   mov   reg[DEC_CR0], 0x00                                          ; Disable dedicated PWM gating the Comp0
   and   reg[CMP_CR0], ~0x01                                         ; Disable dedicated PWM as the interrupt source
...
IF (CSD_CAPACITOR_PIN & CSD_CAPACITOR_P0_1)
   or    reg[CSD_AMUX_CFG_REG], 0x17      ; Capacitor on P0.1  and muxbus clocked from Row0Broadcast
...
_CSD_ScanAllSensors:
   RAM_PROLOGUE RAM_USE_CLASS_4
   mov   A, (CSD_TotalSensorCount - 1)                ; Check if key value in range.
...
ENDIF
   ...
   or    reg[CSD_RB_PORT_DM0], CSD_RB_PIN_MASK		; Change Drive Mode to Strong
   and   reg[CSD_RB_PORT_DM1], ~CSD_RB_PIN_MASK
   M8C_SetBank0
   and   reg[CSD_RB_PORT_DM2], ~CSD_RB_PIN_MASK
...
.Pause:
   ...
   M8C_SetBank1
   or    reg[CSD_RB_PORT_DM0], CSD_RB_PIN_MASK       ; Change Drive mode to Open- Drain low
   or    reg[CSD_RB_PORT_DM1], CSD_RB_PIN_MASK
   M8C_SetBank0
   or    reg[CSD_RB_PORT_DM2], CSD_RB_PIN_MASK
