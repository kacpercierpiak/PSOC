...
export   I2C_fSendRepeatStart
export  _I2C_fSendRepeatStart
export   I2C_fSendStart
export  _I2C_fSendStart
export   I2C_SendStop
export  _I2C_SendStop
; export the following 3 functions for C-implementations large or small mem models 
export  _I2C_fReadBytes
export  _I2C_bWriteBytes
export  _I2C_bWriteCBytes
...
export    I2C_bReadBusStatus
export   _I2C_bReadBusStatus


IF SYSTEM_SMALL_MEMORY_MODEL

export   I2C_fReadBytes
export   I2C_bWriteBytes
export   I2C_bWriteCBytes
...
_I2C_bStatus:
 I2C_bStatus:                          blk   1  ; Status during transfers

AREA UserModules (ROM, REL)
;-----------------------------------------------
;  EQUATES
;-----------------------------------------------
I2C_SLAVE_ACKed:                       equ 0x01 ; This bit set if Slave ACKed Master
I2C_SEND_ACK:                          equ 0x10 ; If this flag set, Master should send ACK


.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_fReadBytes
;
;  DESCRIPTION:
...
_I2C_fReadBytes:			      
	...
	mov   X, SP
I2CMSCR_NotReady1:
    mov    A, [I2C_RsrcStatus]                             ; test to see if previous command started ISR
    tst    [I2C_RsrcStatus], I2CHW_ISR_ACTIVE
    ;mov   A, reg[I2C_MSCR]                                ;read the mscr register to look for pending master operations
    ...
    or    [I2C_RsrcStatus], I2CHW_ISR_ACTIVE               ; flag set here and cleared by ISR
    ...
    mov   [I2C_SlaveAddr], A                               ; preserve addr+r/w state for the ISR to use
    mov   reg[I2C_DR], A                                   ; put the write addr into the I2C_DR reg
    mov   [I2C_bStatus],0x00                               ; Clear all flags
    mov   A, [X+RxMode]                                    ; place the RxMode in status so ISR can access it
    mov   [I2C_bStatus],A
    ;
    ;we must now initialize a read buffer using I2C_InitWrite
    ;
    ...
    call  I2C_InitWrite                                    ;sets the addr and byte count to write to
    add    SP, -3
    pop    X                                               ;restore X to be used for the rest of this routine
    
    mov   A,[I2C_SlaveAddr]
    tst   [x+RxMode],I2C_RepStart
    jnz   DoRestartRx
    tst   reg[I2C_SCR], I2C_BYTE_COMPL                     ; indicates the I2C bus is stalled
    jnz   DoRestartRx
...
I2CMSCR_NotReady2:
    ...
    or    [I2C_RsrcStatus], I2CHW_ISR_ACTIVE               ; flag set here and cleared at end of ISR
    mov   A, [X + TxSlaveAddr]
    asl   A                                                ; Shift address to the left to make
                                                           ; a complete byte with the R/W bit.
    ...
    ;
        ...
        push   A
        call  I2C_InitRamRead                              ;sets the addr and byte count to write to
        add SP, -3
        pop X                                                   ;restore X to be used for the rest of this routine
    ;and  [I2C_RsrcStatus],~I2C_READFLASH                  ;shouldn't be needed should have been taken care of in InitRamRead routine


    mov   A,[I2C_SlaveAddr]
    tst   [x+TxMode],I2C_RepStart
    jnz   DoRestartTx
    tst   reg[I2C_SCR], I2C_BYTE_COMPL                     ; indicates the I2C bus is stalled
    jnz   DoRestartRx
    call  I2C_DoStart                                      ; Send a start and address.
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_bWriteCBytes
;
;  DESCRIPTION:
...
I2CMSCR_NotReady3:
    ...
    ;and   A, 0x0f                                           ;only look at the lower bits
        ...
        or    [I2C_RsrcStatus], I2CHW_ISR_ACTIVE           ;lag set here but cleared in ISRf
    mov   A, [X + TxSlaveAddr]
    asl   A                                                ; Shift address to the left to make
                                                           ; a complete byte with the R/W bit.
                                                           ; The ASL takes care of clearing bit 0.
    ...
    ;
        ...
        call  I2C_InitFlashRead                            ;sets the addr and byte count to write to
        add SP, -4
        pop X                                                   ;restore X to be used for the rest of this routine

    ;or  [I2C_RsrcStatus],I2C_READFLASH
    mov   A,[I2C_SlaveAddr]
    tst   [x+TxMode],I2C_RepStart                          ; Check if a Start or RepeatStart
...
I2C_DoBufferRepeatStart:
    ...
    mov   reg[I2C_DR], A
        mov   [I2C_SlaveAddr], A
        ;tst      reg[I2C_MSCR], I2CM_MASTEROP             ;do we even have control of the bus?
        ;jz    notBusMasterErr
    mov   A, reg[I2C_MSCR]                                 ;read the mscr register to look for pending master operations
    and   A, 0x0f                                          ;only look at the lower bits
        jz    BusIdleSendStart
        ...
        ;SetI2C_SCR I2CM_RESTRT
    ;mov   reg[I2C_MSCR], I2CM_RESTRT
        ;SetI2C_SCR          I2C_TX                                ;even though the restart has been requested the state
        ;mov      reg[I2C_SCR], I2C_TX                     ;even though the restart has been requested the state

;IF I2C_THROTTLE_CLK_RATE
        SetI2C_MSCR I2CM_RESTRT
;ELSE
;    mov   reg[I2C_MSCR], I2CM_RESTRT
;ENDIF
    tst   reg[I2C_SCR], I2C_TX
    jnz   I2C_RestartRecieve

;IF I2C_THROTTLE_CLK_RATE
    SetI2C_SCR I2C_TX                                      ;even though the restart has been requested the state
;ELSE
;    mov   reg[I2C_SCR], I2C_TX                            ;send Ack
;ENDIF
...
;IF I2C_THROTTLE_CLK_RATE
    SetI2C_SCR 0                                           ;even though the restart has been requested the state
;ELSE
;    mov   reg[I2C_SCR], 0                                 ;send Ack
;ENDIF
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_fSendRepeatStart
;
;  DESCRIPTION:
...
;
;  ARGUMENTS:
;    I2C_bAddr - Contains the slave address and transfer direction.
;
;  RETURNS:
;    I2C_bSTatus - nonZero if a slave responds to a request. Zero otherwise
...
;    must be followed directly by I2C_start.
;
;-----------------------------------------------------------------------------
 I2C_fSendRepeatStart:
_I2C_fSendRepeatStart:
    RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2C_bStatus
    ;wait if the bus is already busy...
    push  A
    mov   [I2C_bStatus], 0x00
I2CMSCR_NotReady4:
    mov    A, [I2C_RsrcStatus]                             ; test to see if previous command started ISR
    tst    [I2C_RsrcStatus], I2CHW_ISR_ACTIVE
    ;mov   A, reg[I2C_MSCR]                                ;read the mscr register to look for pending master operations
...
;IF I2C_THROTTLE_CLK_RATE
    SetI2C_MSCR I2CM_RESTRT
    SetI2C_SCR  0                                          ;getting a restart out requires a 0 into the SCR reg
;ELSE
;    mov   reg[I2C_MSCR], I2CM_RESTRT
;    mov   reg[I2C_SCR], 0                                 ;getting a restart out requires a 0 into the SCR reg
;ENDIF

WaitRepStrtCompl:
    mov   A,  reg[I2C_SCR]
    tst   reg[I2C_SCR],I2C_BYTE_COMPL                      ; Test to see if Slave ACKed
    jz    WaitRepStrtCompl
    tst   reg[I2C_SCR], I2C_LST_BIT
    jnz   Err_Exit_RepStart
    mov   [I2C_bStatus], 0x01
    mov   A, 01
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
Err_Exit_RepStart:
 ...
 notBusMaster1:
    ;SetI2C_MSCR 0
    ;mov reg[I2C_MSCR], 0                                  ;we certainly cant restart if we've not Master
...
I2CMSCR_NotReady5:
    ...
    tst   reg[I2C_SCR],I2C_BYTE_COMPL                      ;If there is a pending BYTE_COMPL here
...
I2C_SndWRStart:
    mov   reg[I2C_DR], A
    mov   [I2C_SlaveAddr], A
    ;SetI2C_MSCR, I2CM_SNDSTRT
    ;mov   reg[I2C_MSCR], I2CM_SNDSTRT
...
;    I2C_bData - Contains data to be transmitted.
;
;  RETURNS:
;    I2C_bStatus - Cleared if a slave responds to a request. Set otherwise
;
;  SIDE EFFECTS:
;    The A and X registers may be modified by this or future implementations
;    of this function.  The same is true for all RAM page pointer registers in
...
_I2C_fWrite:
    ...
    ; disable the interrupt
        ; *** NOT REENABLED ***
        ;
    mov   reg[I2C_DR],A                                    ; Put data in Data Reg
    M8C_DisableIntMask I2C_INT_REG, I2C_INT_MASK
    mov   A,  reg[I2C_MSCR]                                ;MSCR into A incase there is an error here
    tst   reg[I2C_MSCR],I2CM_MASTEROP                      ;Do we have control of the bus?
    jz    Err_Exit_fWrite

    mov   [I2C_bStatus],0x00                               ; Clear ACK flag
    ;SetI2C_SCR I2C_TX
    ;mov   reg[I2C_SCR], I2C_TX                            ; Put data in Data Reg

;IF I2C_THROTTLE_CLK_RATE
        SetI2C_SCR I2C_TX                                  ; Put data in Data Reg
;ELSE
;    mov   reg[I2C_SCR], I2C_TX                            ; Put data in Data Reg
;ENDIF
...
I2C_write:
_I2C_write:
;    mov   reg[I2C_SCR],A                                  ; Put data in bData
;   jmp   I2C_get_ack                                      ; This jump is not required since it falls
...
;
;  RETURNS:
;    Sets flag in I2C_bStatus if ACKed by Slave.  !!!
;
;  SIDE EFFECTS:
;    The A and X registers may be modified by this or future implementations
;    of this function.  The same is true for all RAM page pointer registers in
...
_I2C_get_ack:
    RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2C_bStatus
    and [I2C_bStatus], ~I2C_SLAVE_ACKed
    tst reg[I2C_SCR], I2C_BYTE_COMPL
    jnz I2C_get_ack
    tst reg[I2C_SCR], I2C_LST_BIT
    jnz notAcked
    or [I2C_bStatus], I2C_SLAVE_ACKed
notAcked:
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
;    I2C_bStatus - Set for no ack to be followed by stop.
;    Clear for ack to indicate more data to follow.
;
;  RETURNS:
;    I2C_bData - Contains received data.
;
;  SIDE EFFECTS:
;    The A and X registers may be modified by this or future implementations
;    of this function.  The same is true for all RAM page pointer registers in
...
;          CUR_PP
;
;    Must be followed by I2C_put_ack.
;    The I2C interrupt should be disabled since this routine will poll the
;    reg[I2C_SCR] to determine when a byte is available
;
;   WARNING: this routine will disable the I2C interrupt.  It will wait until the I2C_BYTE_COMPLETE
;    flag is set to return.
;
...
;
    ...
    ;SetI2C_SCR 0
    ;mov   reg[I2C_SCR], 0                                 ;clear the I2C_ADDR bit (to unstall SCL) & wait for next byte

;IF I2C_THROTTLE_CLK_RATE
    SetI2C_SCR 0                                           ;clear the I2C_ADDR bit (to unstall SCL) & wait for next byte
;ELSE
;    mov   reg[I2C_SCR], 0                                 ;clear the I2C_ADDR bit (to unstall SCL) & wait for next byte
;ENDIF
                                                           ; through to I2C_put_ack.
...
WaitRXByteCompl:
    ...
    mov   A, reg[I2C_DR]                                   ; Record data received
    ;SetI2C_SCR I2C_ACKOUT
    ;mov   reg[I2C_SCR], I2C_ACKOUT                        ; Record data received

;IF I2C_THROTTLE_CLK_RATE
    SetI2C_SCR I2C_ACKOUT                                  ; Record data received
;ELSE
;    mov   reg[I2C_SCR], I2C_ACKOUT                        ; Record data received
;ENDIF

    mov   [I2C_bStatus],0x01                               ; Clear ACK flag
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
;     the I2C_TX bit in  reg[I2C_SCR].  In either case this is a zero written to
;     reg[I2C_SCR].
;
 I2C_SendStop:
_I2C_SendStop:
    ...
    ;mov  reg[I2C_SCR], 0
;IF I2C_THROTTLE_CLK_RATE
    SetI2C_SCR 0                                           ; Put data in Data Reg
;ELSE
;    mov   reg[I2C_SCR], 0                                 ; Put data in Data Reg
;ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_bReadBusStatus
;
;  DESCRIPTION:
...
;     returned in A.
;       I2C_RepStart:                       equ  0x01
;       I2C_NoStop:                         equ  0x02
;       I2C_NAKnextWr:                      equ  0x04
;
;  SIDE EFFECTS:
;    The A and X registers may be modified by this or future implementations
;    of this function.  The same is true for all RAM page pointer registers in
...
_I2C_bReadBusStatus:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >I2C_bStatus				                        ;Set the Page Pointer for LMM
   mov   A, [I2C_bStatus]                      ;return the status in A
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
