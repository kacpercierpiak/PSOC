...
export    I2C_InitWrite
export   _I2C_InitWrite
export    I2C_InitRamRead
export   _I2C_InitRamRead
export    I2C_InitFlashRead
export   _I2C_InitFlashRead
export    I2C_bReadI2CStatus
export   _I2C_bReadI2CStatus
export    I2C_ClrRdStatus
export   _I2C_ClrRdStatus
export    I2C_ClrWrStatus
export   _I2C_ClrWrStatus
...
WrBufLo: equ -3
    ...
    and    [I2C_bStatus], ~fI2C_NAKnextWr                            ;reset the nak-next-written-byte flag.
IF SYSTEM_LARGE_MEMORY_MODEL
    mov   A, [X + WrBufHi]                                           ;move wrbuf addr to A
    mov   [pI2C_Write_BufHI], A
ENDIF
    mov   A, [X + WrBufLo]                                           ;move wrbuf addr to A
    mov   [pI2C_Write_BufLO], A
    mov   A, [X + WrCnt]                                               ;move Write_count to A
    mov   [I2C_Write_Count], A
    cmp   A, 01                                                        ;if data buffer is one byte long or less
    jz    I2C_SetNak
    jc    I2C_SetNak
    and   [I2C_RsrcStatus], ~0x70                                    ;clear the 0x10, 0x20 (Write status bits)
    jmp   I2Cs_1_ResetFlgs

I2C_SetNak:
    or    [I2C_bStatus], fI2C_NAKnextWr                              ;set the nak-next-written-byte flag.
    and   [I2C_RsrcStatus], ~0x70                                    ; clear the 0x10, 0x20 (Write status bits)
    jmp   I2Cs_1_ResetFlgs

I2Cs_1_ResetFlgs:
    pop A
    and A, I2C_INT_MASK                                                ; Only enable if it was previously enabled
    jz  . + 5
...
RdBufLo:   equ   -3
    ...
    RAM_PROLOGUE RAM_USE_CLASS_2
	RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2C_Read_Count
    push  X
	mov   X, SP
    dec   X                                                          ;set up the pointer for correct param access
    push  A
...
IF SYSTEM_LARGE_MEMORY_MODEL
    ...
    mov   [pI2C_Read_BufHI], A
ENDIF
    mov   A, [X + RdBufLo]                                             ; move rdbuf addr to A
    mov   [pI2C_Read_BufLO], A
    mov   A, [X + RdCnt]                                               ; move RamRead_count to A
    mov   [I2C_Read_Count], A
    dec   [I2C_Read_Count]                                           ; since we decrement through zero...
    and   [I2C_RsrcStatus], ~0x0f                                    ; clear the lower 4 (read status bits)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_InitFlashRead
;
;  DESCRIPTION:
...
IF I2C_READ_FLASH
    RAM_PROLOGUE RAM_USE_CLASS_4
    RAM_PROLOGUE RAM_USE_CLASS_2
    RAM_SETPAGE_CUR >pI2C_Read_BufLO   
	push  X
	mov   X, SP
    dec   X                                                            ;set up the pointer for correct param access
    push  A
    ...
    mov   [I2C_Read_CountHI], A

    dec   [I2C_Read_Count]                                           ; since this will count throu zero
    jnc    . + 4                                                       ; only Read_CountHI if Read_Count rolled to 0xff
    dec   [I2C_Read_CountHI]

    and   [I2C_RsrcStatus], ~0x07                                    ; clear the lower 3 (read status bits)
    or    [I2C_RsrcStatus],I2CHW_READFLASH                           ; set the flash status bit
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_bReadI2CStatus
;
;  DESCRIPTION:
...
_I2C_bReadI2CStatus:
    ...
    mov   A, [I2C_RsrcStatus]                                        ;return the status in A
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
_I2C_ClrWrStatus:
    RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2C_RsrcStatus
    and   [I2C_RsrcStatus], ~0x70                                    ; clear bits 10,20 (write status bits)
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
